...
public:
    GeneralMember(int type, string id, string pw, string name, string registrationNumber);  // 타입, 아이디, 비밀번호, 이름, 주민등록번호를 받아 GeneralMember 객체 생성
    ...
    Application* deleteApplication(Application* delApp);    // 인자로 받은 Application 객체를 collection에서 삭제
    virtual string getName();                               // 회사 이름 반환
    virtual string number();                                // 주민등록번호 반환
};
