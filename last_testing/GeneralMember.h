...
private:
    ...
    ApplicationCollection app_collection_;

public:
    GeneralMember(int type, string id, string pw, string name, string registration_number);  
                                                            // 타입, 아이디, 비밀번호, 이름, 주민등록번호를 받아 GeneralMember 객체 생성
    vector<Application*>& getApplicationList();             // Application* 벡터(리스트) 참조 반환
    void addApplication(Application* new_app);              // 인자로 받은 Application 객체를 collection에 추가
    void deleteApplication(Application* del_app);           // 인자로 받은 Application 객체를 collection에서 삭제
